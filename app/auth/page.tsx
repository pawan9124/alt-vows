...
function AuthContent() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === 'signup') {
                ...
                // (with email confirmation enabled, it doesn't throw an error for security)
    ...
    const handleForgotPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(friendlyError(err.message || 'Failed to send reset email.'));
        } finally {
            setLoading(false);
        }
    };
        ...
        <main className="min-h-screen bg-[#08070A] flex">
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center px-4 sm:px-8 py-16 pt-20 relative">
                ...
                <div className="w-full max-w-[420px] relative z-10">
                    ...
                    >
                        {/* Pill Toggle — hidden in forgot mode */}
                        {mode !== 'forgot' && (
                        ...
                        {mode === 'forgot' && (
                            <div className="mb-6">
                                <h3 className="text-[#F0EBE0] font-semibold text-lg mb-1">Reset Password</h3>
                                <p className="text-[#6A6168] text-sm">We'll email you a link to reset your password.</p>
                            </div>
                        )}
                        ...
                        {mode === 'forgot' ? (
                            <form onSubmit={handleForgotPassword} className="space-y-5">
                                ...
                                <button
                                    type="button"
                                    onClick={() => { setMode('signin'); setError(null); setSuccessMessage(null); }}
                                    className="w-full text-center text-[#9A9198] text-sm hover:text-[#D4A843] transition-colors"
                                >
                                    ← Back to Sign In
                                </button>
                            </form>
                                    ...
                                    <div>
                                        ...
                                        {mode === 'signin' && (
                                            <button
                                                ...
                                                className="mt-2 text-[13px] text-[#6A6168] hover:text-[#D4A843] transition-colors"
                                            >
                                                Forgot password?
                                            </button>
                                        )}
